            ...
            if self.step_callback is not None:
                try:
                    ...
                    for _m in reversed(messages):
                        if _m.get("role") == "tool":
                            try:
                                result = _m.get("content", "")
                                tool_name = ""
                                for tc in reversed(messages):
                                    if tc.get("role") == "assistant" and tc.get(
                                        "tool_calls"
                                    ):
                                        for tc_item in tc["tool_calls"]:
                                            if tc_item.get("id") == _m.get(
                                                "tool_call_id"
                                            ):
                                                tool_name = tc_item.get(
                                                    "function", {}
                                                ).get("name", "")
                                                break
                                        if tool_name:
                                            break
                                if tool_name:
                                    is_err, _ = _detect_tool_failure(tool_name, result)
                                    if is_err:
                                        tool_errors += 1
                            except Exception:
                                pass
                    self.step_callback(api_call_count, prev_tools, tool_errors)

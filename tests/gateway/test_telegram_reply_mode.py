...
class TestShouldThreadReply:
    ...
    def test_no_reply_to_returns_false(self, adapter_factory):
        adapter = adapter_factory(reply_to_mode="first")
        assert adapter._should_thread_reply("chat-123", None, 0) is False
        assert adapter._should_thread_reply("chat-123", "", 0) is False

    def test_off_mode_never_threads(self, adapter_factory):
        adapter = adapter_factory(reply_to_mode="off")
        assert adapter._should_thread_reply("chat-123", "msg-123", 0) is False
        assert adapter._should_thread_reply("chat-123", "msg-123", 1) is False
        assert adapter._should_thread_reply("chat-123", "msg-123", 5) is False

    def test_first_mode_only_first_chunk(self, adapter_factory):
        adapter = adapter_factory(reply_to_mode="first")
        assert adapter._should_thread_reply("chat-123", "msg-123", 0) is True
        assert adapter._should_thread_reply("chat-123", "msg-123", 1) is False
        assert adapter._should_thread_reply("chat-123", "msg-123", 2) is False
        assert adapter._should_thread_reply("chat-123", "msg-123", 10) is False

    def test_all_mode_all_chunks(self, adapter_factory):
        adapter = adapter_factory(reply_to_mode="all")
        assert adapter._should_thread_reply("chat-123", "msg-123", 0) is True
        assert adapter._should_thread_reply("chat-123", "msg-123", 1) is True
        assert adapter._should_thread_reply("chat-123", "msg-123", 2) is True
        assert adapter._should_thread_reply("chat-123", "msg-123", 10) is True
...
class TestSendWithReplyToMode:
    ...
    async def test_off_mode_no_reply_threading(self, adapter_factory):
        ...
        await adapter.send("chat-123", "test content", reply_to="msg-999")
    ...
    async def test_no_reply_to_param_no_threading(self, adapter_factory):
        ...
        await adapter.send("chat-123", "test content", reply_to=None)
    ...
    async def test_single_chunk_respects_mode(self, adapter_factory):
        ...
        await adapter.send("chat-123", "test", reply_to="msg-999")

...
class DraftStreamLoop:
    ...
    async def _send_pending(self) -> None:
        ...
        if self._state.message_id is None and len(text) < self._initial_min_chars:
            return
        ...
        msg_id = None if force_new else self._state.message_id
...
class TelegramDraftStream:
    ...
    async def _send_update(self) -> Optional[Dict[str, Any]]:
        ...
        if self._state.message_id is None and len(text) < 30:
            return None
...
class ReasoningLaneCoordinator:
    ...
    def _get_thinking_re(cls):
        """Lazy compile the thinking tag regex."""
    ...
    def split_text(cls, text: str) -> Dict[str, str]:
        ...
        has_thinking = bool(re_pattern.search(text))
        ...
        if has_thinking:
            ...
            for match in re_pattern.finditer(text):
                ...
                is_closing = "/" in match.group(1) or "/" in match.group(0)
                
                if not in_thinking and not is_closing:
                    # Opening tag - save preceding as answer
                    if current:
                        answer_parts.append("".join(current))
                        current = []
                    in_thinking = True
                elif in_thinking and is_closing:
                    # Closing tag - save current as reasoning
                    reasoning_parts.append("".join(current))
                    current = []
                    in_thinking = False
                elif in_thinking:
                    # Content inside thinking tags
